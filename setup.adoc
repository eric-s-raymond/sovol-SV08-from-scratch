...
// batchspell: add tmc toolhead unix ZOFFSET HOWTO adoc gerGo
...
Optional step: Use the glue gun to secure the four JST
connectors. (You won't need to take the shield protecting the
electronics off; the connectors are the ones that mate with the
motors.)

After initial setup, I had two epiosodes of autohome failure that were
fixed by reseating the stepper connectors.  This is not a
commonly-reported bug, but you may want to hedge against it anyway.
...
connections sand changing two Klipper configuration variables. To
understand how, you have to know what is going on when auto-home
fails.
...
stepper_x.driver_sgthrs and tmc2209.stepper_y variables. Their legal
range is 0-255. You're going to perform a bisection search until you
find values at which homing the toolhead kisses both travel limits,
coming to a gentle but consistent stop.
...
Important: after building the enclosure, check that all the stepper
motor connectors are firmly seated. Mine got loosened, leading to
chatterbox failure.
