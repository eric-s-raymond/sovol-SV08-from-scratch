...
. The magnets holding on the tool-head cover aren't very
  strong. Check if your toolhead is rescent enough to have a retaining 
...
. Inspect the JST connectors plugged into the four stepper motors on
  the underside of the base atnear its four corners. You won't need to
...
turned into weeks of trobleshooting and false starts.
